...
static void adder(void * mutex, unsigned long addernumber)
{
...
int maths (int data1, char **data2)
{
        ...
        // count initialised to 1 -> 1 thread allowed in this zone at a time
        mutex = sem_create("mutex", 1);

        if (mutex == NULL) {
                panic("maths: sem create failed (mutex)");
        }
        ...
        for (index = 0; index < NADDERS; index++) {
               //P(mutex);
                error = thread_fork("adder thread", NULL, &adder, mutex, index);
